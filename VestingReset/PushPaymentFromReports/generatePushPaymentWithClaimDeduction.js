...
function readTransactionFiles(fileNames) {
    ...
    console.log(`\n📈 Summary: ${allFilesData.length} file(s) loaded, ${totalTransactions} total transactions`);
...
function readClaimsReport(filename) {
    try {
        ...
        console.log(`✅ Successfully read claims report: ${filename}`);
        console.log(`📊 Found ${claimsReport.totalUsers} users with ${claimsReport.totalClaims} claims`);
        return claimsReport;
    } catch (error) {
        if (error.code === 'ENOENT') {
            console.log(`⚠️  Claims report not found: ${filename}`);
            console.log(`   Run 'npm run generate-claims-report' first to generate the report`);
            return null;
        }
        console.error(`❌ Error reading claims report ${filename}:`, error.message);
        throw error;
    }
}
